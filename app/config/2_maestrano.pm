...
my $mno_sso_host = 'http://localhost:3000';
...
$mno_settings->{sso_intranet_mode} = 0;

# Maestrano X509 Certificate
$mno_settings->{sso_x509_certificate} = <<CERTIFICATEDOC;
-----BEGIN CERTIFICATE-----
...
# Write certificate
my $path = abs_path(__FILE__ . '/../') . '/cacert.pem';
open (CACERTFILE, ">" . $path);
print CACERTFILE $mno_settings->{sso_x509_certificate};
close (CACERTFILE);

$mno_settings->{sso_cacert_path} = $path;
